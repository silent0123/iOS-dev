...
- (void)viewWillAppear:(BOOL)animated 
{
  ...
  if (_mode == KKPasscodeModeSet || _mode == KKPasscodeModeChange) {
		...
		*/
		...
		[_boxes addObject:[self boxes]];
		UIView *boxesView = [[UIView alloc] initWithFrame:CGRectMake(self.view.bounds.size.width * 0.5 - 71.0 * kPasscodeBoxesCount * 0.5 + 5, 40, 71.0 * kPasscodeBoxesCount, kPasscodeBoxHeight)];
		boxesView.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleRightMargin;
		for (int i = 0; i < [[_boxes lastObject] count]; i++) {
			[boxesView addSubview:[[_boxes lastObject] objectAtIndex:i]];
		}
		...
		[_boxes addObject:[self boxes]];
		UIView *boxesConfirmView = [[UIView alloc] initWithFrame:CGRectMake(self.view.bounds.size.width * 0.5 - 71.0 * kPasscodeBoxesCount * 0.5 + 5, 40, 71.0 * kPasscodeBoxesCount, kPasscodeBoxHeight)];
...
- (void)incrementFailedAttemptsLabel
{
  AudioServicesPlaySystemSound(kSystemSoundID_Vibrate);
	...
	_failedAttemptsLabel.frame = _failedAttemptsView.frame = CGRectMake((self.view.bounds.size.width - (size.width + 40.0)) / 2, 150 + 40, size.width + 40.0, size.height + 10.0);
...
- (void)moveToNextTableView
{
	...
	for (int i = 0; i < kPasscodeBoxesCount; i++) {
		[[[_boxes objectAtIndex:_currentPanel] objectAtIndex:i] setImage:[UIImage imageNamed:@"box_empty"]];
	}
		...
		if (_mode == KKPasscodeModeSet) {
			...
			if ([textField isEqual:_enterPasscodeTextField]) {
				...
				if ([passcode isEqualToString:_setPasscodeTextField.text]) {
					...
					_passcodeConfirmationWarningLabel.frame = CGRectMake(0.0, 132.0 + 40, self.view.bounds.size.width, 60.0);
				} else {
					_passcodeConfirmationWarningLabel.text = @"";
					_passcodeConfirmationWarningLabel.frame = CGRectMake(0.0, 146.0 + 40, self.view.bounds.size.width, 30.0);
					[self moveToNextTableView];
				}
			} else if ([textField isEqual:_confirmPasscodeTextField]) {
				if (![_confirmPasscodeTextField.text isEqualToString:_setPasscodeTextField.text]) {
