...
- (void)viewWillAppear:(BOOL)animated {
    //[self.navigationController.navigationBar.topItem setTitle:NSLocalizedString(@"SingleFileHistory", @"")];
}
...
- (void)setCellForOp:(USAVTableViewCell *) cell inPath:(NSIndexPath *)indexPath
{
    ...
    //现实Permission细节
    if ([operation isEqualToString:@"Change Permission"]) {
    ...
    //显示剩余次数
    if ([operation isEqualToString:@"Decrypt File"]) {
        NSArray *comp = [[singleLog objectForKey:@"Content"] componentsSeparatedByString:@","];
    ...
    cell.operation.text =  [NSString stringWithFormat:@"Operation: %@", operation];
    cell.separatorInset = UIEdgeInsetsZero;
    cell.content.text = [NSString stringWithFormat:@"Operator: %@",[singleLog objectForKey:@"Doer"]];
...
//    cell.contentView.backgroundColor = indexPath.row & 1 ? [UIColor colorWithRed:0.91 green:0.94 blue:0.97 alpha:1]:[UIColor whiteColor];
    ...
    if (r == 0) {
        ...
        if (r==517) {
            cell.result.text = @"Permission Denied";
