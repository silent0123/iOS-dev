...
- (void)viewDidLoad
{
    ...
    //self.navigationController.navigationBar.topItem.title = NSLocalizedString(@"Preview File", nil);
    ...
    */ 
    ...
    } else if ([[self.fullFilePath pathExtension] caseInsensitiveCompare:@"aac"] == NSOrderedSame) {
        //录音播放
        [self playVoiceMessage];
    } else {
    ...
    if ([[self.fullFilePath pathExtension] isEqualToString:@"usavm"] ||
        [[self.fullFilePath pathExtension] isEqualToString:@"aac"]) {
    ...
    self.replyTextView.layer.cornerRadius = 5;   //圆角
    ...
    [self.voiceRecordingSwitchBtn.layer setCornerRadius:5];
...
#pragma mark - Voice message player
...
#pragma mark - Voice Message Btn
- (void)switchBetweenVoiceAndText: (id)sender {
    
    if ([((UIButton *)sender).titleLabel.text isEqualToString:@"Voice"]) {
        
        [self.voiceRecordingSwitchBtn setTitle:@"Text" forState:UIControlStateNormal];
        [self.voiceRecordingBtn setHidden:NO];
        
    } else {
        [self.voiceRecordingSwitchBtn setTitle:@"Voice" forState:UIControlStateNormal];
        [self.voiceRecordingBtn setHidden:YES];
...
- (void)voiceRecordingTimerTick {
    ...
    [self.voiceRecordingBtn setTitle:[NSString stringWithFormat:@"Recording %.2zi:%.2zi", minutes, seconds] forState:UIControlStateHighlighted];
}
...
- (void)keyboardWillHide: (NSNotification *)notification {
    ...
    self.navigationController.navigationBar.topItem.rightBarButtonItem = self.replyBtn;
...
-(void)viewDidAppear:(BOOL)animated
{
    ...
    [self.navigationController.navigationBar.topItem setTitle:[self.fullFilePath lastPathComponent]];
}
...
- (void)alertView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex {
    ...
    if (alertView.tag == 100) {
        ...
        if (buttonIndex == 1) {
            ...
            self.alert = [SGDUtilities showLoadingMessageWithTitle:NSLocalizedString(@"Sending", @"") message:NSLocalizedString(@"Permission will be assigned automatically", @"") delegate:self];
        }
        
        if (buttonIndex == 2) {
            [self performSegueWithIdentifier:@"ReplyPermissionSegue" sender:self];
        }
    }
}
 */
