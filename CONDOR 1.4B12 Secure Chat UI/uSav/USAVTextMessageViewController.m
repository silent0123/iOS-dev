...
- (void)viewDidAppear:(BOOL)animated {
    ...
    [self.navigationController.navigationBar.topItem setTitle:NSLocalizedString(@"Secure Message", nil)];
...
- (void)viewDidLoad {
    ...
    self.voiceRecordingCustomView.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"Function_bg"]];
    self.voiceRecordingBtn.layer.masksToBounds = YES;
    [self.voiceRecordingBtn.layer setCornerRadius:5];
    ...
    if([[[UIDevice currentDevice].model substringToIndex:4] isEqualToString:@"iPad"]){
        [self.voiceRecordingBtn.titleLabel setFont:[UIFont systemFontOfSize:12]];
        [self.voiceRecordingHintLabel setFont:[UIFont systemFontOfSize:10]];
        [self.voiceRecordingTimeLabel setFont:[UIFont boldSystemFontOfSize:28]];
        [self.voiceRecordingBtn setFrame:CGRectMake(self.voiceRecordingBtn.frame.origin.x + 13, self.voiceRecordingBtn.frame.origin.y - 76, self.voiceRecordingBtn.frame.size.width - 26, self.voiceRecordingBtn.frame.size.height - 10)];
        [self.voiceRecordingTimeLabel setFrame:CGRectMake(self.voiceRecordingTimeLabel.frame.origin.x, self.voiceRecordingBtn.frame.origin.y - 82, self.voiceRecordingTimeLabel.frame.size.width, self.voiceRecordingTimeLabel.frame.size.height)];
        [self.voiceRecordingIndicator setFrame:CGRectMake(self.voiceRecordingIndicator.frame.origin.x, self.voiceRecordingIndicator.frame.origin.y - 50, self.voiceRecordingIndicator.frame.size.width, self.voiceRecordingIndicator.frame.size.height)];
        [self.voiceRecordingHintLabel setFrame:CGRectMake(self.voiceRecordingHintLabel.frame.origin.x, self.voiceRecordingHintLabel.frame.origin.y - 72, self.voiceRecordingHintLabel.frame.size.width, self.voiceRecordingHintLabel.frame.size.height)];
    }
    ...
    //用Label实现PlaceHolder
    self.placeHolderLabel = [[UILabel alloc] initWithFrame:CGRectMake(10, 5, 100, 20)];
...
- (void)keyboardWillShow:(NSNotification *)notification {
    ...
    if([[[UIDevice currentDevice].model substringToIndex:4] isEqualToString:@"iPad"]){
        ...
        if (self.keyboardRect.size.height == 0 || self.keyboardRect.origin.y == self.view.frame.size.height) {
            [self animateTextFiled:self.messageTextView up:NO forHeight:self.keyboardRect.size.height + 108];
            
            //voice switch btn
            [self.voiceRecordingSwitchBtn setFrame:CGRectMake(0, self.keyboardRect.origin.y - self.voiceRecordingSwitchBtn.frame.size.height - 64, self.voiceRecordingSwitchBtn.frame.size.width, self.voiceRecordingSwitchBtn.frame.size.height)];
        } else {
            //改变frame，先下降回去
            [self animateTextFiled:self.messageTextView up:NO forHeight:previousHeight];
            [self animateTextFiled:self.messageTextView up:YES forHeight:self.keyboardRect.size.height + 108];
            
            //voice switch btn
            [self.voiceRecordingSwitchBtn setFrame:CGRectMake(0, self.keyboardRect.origin.y - self.voiceRecordingSwitchBtn.frame.size.height - 64, self.voiceRecordingSwitchBtn.frame.size.width, self.voiceRecordingSwitchBtn.frame.size.height)];
        }
        
        previousHeight = self.keyboardRect.size.height + 108;
        
    } else {
        if (self.keyboardRect.size.height == 0 || self.keyboardRect.origin.y == self.view.frame.size.height) {
            
            [self animateTextFiled:self.messageTextView up:NO forHeight:self.keyboardRect.size.height + 18];
            //voice switch btn
            [self.voiceRecordingSwitchBtn setFrame:CGRectMake(0, self.keyboardRect.origin.y - self.voiceRecordingSwitchBtn.frame.size.height - 64, self.voiceRecordingSwitchBtn.frame.size.width, self.voiceRecordingSwitchBtn.frame.size.height)];
//            NSString *output = [NSString stringWithFormat:@"%@, key:%@",NSStringFromCGRect(self.voiceRecordingSwitchBtn.frame), NSStringFromCGRect(self.keyboardRect)];
//            NSLog(output);
        } else {
            //改变frame，先下降回去
            [self animateTextFiled:self.messageTextView up:NO forHeight:previousHeight];
            [self animateTextFiled:self.messageTextView up:YES forHeight:self.keyboardRect.size.height + 18];
...
//            NSLog(output);
        ...
        previousHeight = self.keyboardRect.size.height + 18;
    }
...
//switch button between text & voice
- (IBAction)voiceRecordingSwitchBtnPressed:(id)sender {
    
    if ([((UIButton *)sender).titleLabel.text isEqualToString:@"Switch to Voice Message"]) {
        [((UIButton *)sender) setTitle:NSLocalizedString(@"Switch to Text Message", nil) forState:UIControlStateNormal];
        [self.voiceRecordingCustomView setHidden:NO];
        [self.view bringSubviewToFront:self.voiceRecordingSwitchBtn];
        self.voiceRecoding = [[USAVVoiceRecoding alloc] initWithAudioSession];
    } else {
        [((UIButton *)sender) setTitle:NSLocalizedString(@"Switch to Voice Message", nil) forState:UIControlStateNormal];
        [self textMessageSwitchBtnPressed:sender];
    }
