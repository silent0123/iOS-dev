...
- (void)setOverlayTitle{
    ...
    [_overlayTitle setText:@"Coming Soon"];
}
...
- (void)setOverlayTexts{
    ...
    for(ICETutorialPage *page in _pages){
        ...
        if ([[[page subTitle] text] length]){
            ...
            NSLog(@"%@", NSStringFromCGRect(subTitle.frame));
        }
        // Description.
...
- (UILabel *)overlayLabelWithText:(NSString *)text
    ...
    // SubTitles.
    UILabel *overlayLabel = [[UILabel alloc] initWithFrame:CGRectMake((index  * _windowSize.width),  _windowSize.height - [commonStyle offset], _windowSize.width, TUTORIAL_LABEL_HEIGHT)];
...
- (void)scrollingToNextPageWithOffset:(float)offset{
    ...
    float alphaValue = offset - (NSInteger)offset;
