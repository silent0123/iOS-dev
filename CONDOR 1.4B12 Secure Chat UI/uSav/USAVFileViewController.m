...
//@property (strong, nonatomic) TTOpenInAppActivity *openInAppActivity;   //new for 1.4 B10 later
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    for (NSInteger i = 0; i < [self.view.subviews count]; i++) {
        ...
        if ([[USAVLock defaultLock] isLogin]) {
            ...
            //[self.navigationController.navigationBar.topItem setRightBarButtonItem:self.rightBarLogoutBtn];
            [self.navigationController.navigationBar.topItem setRightBarButtonItem:self.sortBtn];
...
-(void)enableLoginBtn
{
    ...
    [self.navigationController.navigationBar.topItem setRightBarButtonItem:self.rightBarLoginBtn];
}
	...
	if ((obj != nil) && ([obj objectForKey:@"httpErrorCode"] == nil)) {
        ...
        switch (rc) {
            case SUCCESS:
            {
                ...
                //NSLog(@"%d %d", [keyId length], [keyContent length]);
...
#pragma mark sort menu生成
    ...
    sortMenuBtn_4.titleLabel.font = [UIFont boldSystemFontOfSize:14];
    ...
    self.hintLabel.font = [UIFont boldSystemFontOfSize:14];
    
//  [self.navigationController setNavigationBarHidden:NO animated:NO];
    [self.navigationController.navigationBar.topItem setTitle:NSLocalizedString(@"TabHome", @"")];
...
- (void)showLoginStatusCheckFailedAlert {
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Login Timeout" message:@"Please log in CONDOR again" delegate:self cancelButtonTitle:NSLocalizedString(@"Dismiss", nil) otherButtonTitles:nil, nil];
    [alert show];
}

-(void)autoViewing:(NSNotification *) notification {
    self.timer = [NSTimer scheduledTimerWithTimeInterval:2 target:self selector:@selector(previewDelay) userInfo:nil repeats:NO];
...
-(NSInteger)copyfile
{
    ...
    for (NSInteger i = 0; i < [allFile count]; i++) {
        ...
        if ([[singleFile pathExtension] caseInsensitiveCompare:@"usav"] == NSOrderedSame) {
            ...
            NSString *extension = [[singleFile stringByDeletingPathExtension] pathExtension]; //得到实际ext
            NSString *targetFile;
...
-(void)disableBackBtn
{
    ...
    //[self.navigationController.navigationBar.topItem setRightBarButtonItem:self.rightBarLogoutBtn];
    [self.navigationController.navigationBar.topItem setRightBarButtonItem:self.sortBtn];
    //self.navigationController.navigationBar.topItem.leftBarButtonItem = self.photoBtn;
...
{
    //这个是修改过的backBtn
    self.navigationController.navigationBar.topItem.leftBarButtonItem = self.goBackBtn;
}
...
- (void)folderBackBtnPressed:(id)sender {
    ...
    if (self.currentPath.length > self.basePath.length) {
        ...
        [self.navigationController.navigationBar.topItem setTitle:NSLocalizedString(@"FileHomeTitleKey", @"")];
        [self disableBackBtn];
        [self.currentFileList removeAllObjects];
        [self.currentFileList addObjectsFromArray:[self.fileManager contentsOfDirectoryAtPath:self.currentPath error:nil]];
        // [self.tblView reloadData];
...
- (NSInteger)tableView:(UITableView *)tableView
 numberOfRowsInSection:(NSInteger)section {
    ...
    if ([filenameStr isEqualToString:@"Encrypted"]) {
        ...
        [self.navigationController.navigationBar.topItem setTitle:[NSString stringWithFormat:@"%@",
                                                                   NSLocalizedString(@"Files", nil)]];
        
    } else if ([filenameStr isEqualToString:@"PhotoAlbum"]){
        
        [self.navigationController.navigationBar.topItem setTitle:NSLocalizedString(@"Secure Album", nil)];
        
        
    } else if ([filenameStr isEqualToString:@"DecryptedCopy"]){
        [self.navigationController.navigationBar.topItem setTitle:NSLocalizedString(@"Decrypted File", nil)];
        
    } else {
        [self.navigationController.navigationBar.topItem setTitle:[NSString stringWithFormat:@"%@",NSLocalizedString(filenameStr, nil)]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView
    ...
    if (isDirectory) {
        ...
        cell.accessoryType = UITableViewCellAccessoryNone;
        //NSLog(@"%ld %d", (long)indexPath.row + 1, self.selected);
...
#pragma mark 如果是Open In的Message，在显示的时候删除
    if ([filenameStr isEqualToString:[self.encryptPath lastPathComponent]]) {
        ...
        for (NSInteger i = 0; i < [filesArray count]; i ++) {
            if ([[[UsavFileHeader defaultHeader] getExtension:[NSString stringWithFormat:@"%@/%@", self.encryptPath, [filesArray objectAtIndex:i]]] caseInsensitiveCompare:@"usavm"] == NSOrderedSame || [[[UsavFileHeader defaultHeader] getExtension:[NSString stringWithFormat:@"%@/%@", self.encryptPath, [filesArray objectAtIndex:i]]] caseInsensitiveCompare:@"aac"] == NSOrderedSame){
                    ...
                    if (encryptedData) {
                        ...
                        if ([encryptedData writeToFile:targetFullPath atomically:YES]){
                            ...
                            if ([self.currentPath isEqualToString:self.encryptPath]) {
                                [self openFolder:@"Files" Path:self.currentPath];
                            } else if ([self.currentPath isEqualToString:self.photoAlbumPath]) {
                                [self openFolder:@"Secure Album" Path:self.currentPath];
                            }
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    if ([segue.identifier isEqualToString:@"copeople"]) {
        ...
        USAVSecureChatListTableViewController *secureChatListController = segue.destinationViewController;
        secureChatListController.fileViewControllerDelegate = self;
    }
}
...
-(void)openDocumentIn {
    ...
    /*
    self.openInAppActivity = [[TTOpenInAppActivity alloc] initWithView:self.view andRect:CGRectZero];
    self.openInAppActivity.delegate = self;
    UIActivityViewController *activityViewController = [[UIActivityViewController alloc] initWithActivityItems:@[testURL, fullPath, ] applicationActivities:nil];
    
    self.openInAppActivity.superViewController = activityViewController;
    ...
    */
    ...
    BOOL isPresented = [self.docInteractionController presentOpenInMenuFromRect:CGRectZero inView:self.view animated:YES];
...
-(void)previewFile {
    ...
    if (([ext caseInsensitiveCompare:@"pdf"] == NSOrderedSame) ||
        ...
        ([ext caseInsensitiveCompare:@"aac"] == NSOrderedSame)){    //aac是当成录音文件
        ...
        case 3: {
            ...
            for(NSString* name in fileList)
            {
                ...
                NSDictionary* properties = [tempFileManager
                                            ...
                                            error:&error];
                //这里是取header里的extension位
                NSString *type = [[UsavFileHeader defaultHeader] getExtension:[path stringByAppendingPathComponent:name]];
