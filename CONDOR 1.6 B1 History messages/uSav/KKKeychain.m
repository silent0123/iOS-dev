...
+ (BOOL)setString:(NSString*)string forKey:(NSString*)key 
{
  ...
  OSStatus res = SecItemCopyMatching((__bridge_retained CFDictionaryRef) existsQueryDictionary, NULL);       
	...
	if (res == errSecItemNotFound) {
		if (string != nil) {
			...
			res = SecItemAdd((__bridge_retained CFDictionaryRef)addDict, NULL);
			NSAssert1(res == errSecSuccess, @"Recieved %zi from SecItemAdd!", res);
		}
		...
		res = SecItemUpdate((__bridge_retained CFDictionaryRef)existsQueryDictionary, (__bridge_retained CFDictionaryRef)attributeDict);
		NSAssert1(res == errSecSuccess, @"SecItemUpdated returned %zi!", res);
	} else {
		NSAssert1(NO, @"Received %zi from SecItemCopyMatching!", res);
	}
	return YES;
}
...
{  
	...
	[existsQueryDictionary setObject:(__bridge id)kCFBooleanTrue forKey:(__bridge id)kSecReturnData];
	...
	if (res == errSecSuccess) {
		...
		NSAssert1(res == errSecItemNotFound, @"SecItemCopyMatching returned %zi!", res);
	}		
