...
{
    [self.navigationItem setTitle:NSLocalizedString(@"TabBarProfile", @"")];
    
    //return 8;
    return 7;   //temporary hide
}
...
- (NSString *)tableView:(UITableView *)tableView titleForHeaderInSection:(NSInteger)section
{
    if (section == 0) {
        ...
        return NSLocalizedString(@"LanguageSetting", @"");
    }else if(section == 2) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0) {
        ...
        cell.textLabel.text = NSLocalizedString(@"Set Language", @"");
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(indexPath.section == 2) {
        ...
        [self performSegueWithIdentifier:@"LanguageSetting" sender:self];
...
//    }
        ...
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"Warning", nil) message:NSLocalizedString(@"This will delete ALL FILES in your CONDOR account on this device, and can NOT recovery", nil) delegate:self cancelButtonTitle:NSLocalizedString(@"Cancel", nil) otherButtonTitles:NSLocalizedString(@"OkKey", nil), nil];
        //self.alert = alert;
        [alert show];
    }
...
- (void)eraseFilesAndCache {
    ...
    NSString *messagePath = [NSString stringWithFormat:@"%@/%zi/%@", currentPath, [[USAVClient current] uId], @"messages"];
    ...
    //清空Message
    [allFile removeAllObjects];
    [allFile addObjectsFromArray:[fileManager contentsOfDirectoryAtPath:messagePath error:nil]];
    for(NSInteger i = 0; i < [allFile count]; i++){
        NSString *messageFilePath = [NSString stringWithFormat:@"%@/%@", messagePath, [allFile objectAtIndex:i]];   //allFile只是文件名
        [fileManager removeItemAtPath:messageFilePath error:nil];
    }
