...
- (id)initWithFrame:(CGRect)frame subMenus:(NSArray *)menus tip:(NSString *)tip
{
    ...
    if (self)
    {
        ...
        //最后添加标题
        self.topTitle = [[UILabel alloc] initWithFrame:CGRectMake(self.bounds.size.width/1.9 - (210/2), self.bounds.size.height/7.5, 196, (36.0/568.0)*self.bounds.size.height)];
...
- (void)setupSubmenus
{
    ...
    for (NSInteger i = 0; i < 3; ++i)
    {
        ...
        for (NSInteger j = 0; j < 3; ++j)
        {
            ...
            subMenu.center = CGPointMake(103 * j + 62, CGRectGetHeight(self.frame) + i * 125 + 60);
            //第三行下移一行，并且微调位置
            if (i == 2) {
                subMenu.center = CGPointMake(103 * j + 62, CGRectGetHeight(self.frame) + (i + 1) * ((125/568.0)*screenHeight) - ((20/568.0)*screenHeight) );
            }
            //如果是iPhone4，第三行则轻微修正位置
            if (screenHeight == 480 && i == 2) {
                subMenu.center = CGPointMake(103 * j + 62, CGRectGetHeight(self.frame) + (i + 1) * ((125/568.0)*screenHeight) - ((0/568.0)*screenHeight) );
            }
            
            //如果是iPad，则微调位置
            if([[[UIDevice currentDevice].model substringToIndex:4] isEqualToString:@"iPad"]){
                subMenu.center = CGPointMake(101 * j + 62, CGRectGetHeight(self.frame) + i * 100 + 84);
                
                //第三行下移一行，并且微调位置
                if (i == 2) {
                    subMenu.center = CGPointMake(101 * j + 62, CGRectGetHeight(self.frame) + (i + 1) * 125 - 56);
                }
            }   //i*x为紧密度，+的数字为整体位移
