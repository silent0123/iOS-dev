...
static const NSInteger kGDataXMLParseOptions = (XML_PARSE_NOCDATA | XML_PARSE_NOBLANKS);
...
static xmlChar *SplitQNameReverse(const xmlChar *qname, xmlChar **prefix) {
  ...
  NSInteger qnameLen = xmlStrlen(qname);
  for (NSInteger idx = qnameLen - 1; idx >= 0; idx--) {
...
- (NSString *)XMLString {
  ...
  if (xmlNode_ != NULL) {
    ...
    if (buff) {
      ...
      NSInteger result = xmlNodeDump(buff, doc, xmlNode_, level, format);
    ...
    if (xpathCtx) {
      ...
      if (namespaces) {
        ...
        for (NSString *prefix in namespaces) {
          ...
          NSInteger result = xmlXPathRegisterNs(xpathCtx, prefixChars, uriChars);
          if (result != 0) {
#if DEBUG
        ...
        if (nsNodePtr != NULL) {
          for (xmlNsPtr nsPtr = nsNodePtr->ns; nsPtr != NULL; nsPtr = nsPtr->next) {
            ...
            NSInteger result = xmlXPathRegisterNs(xpathCtx, prefix, nsPtr->href);
            if (result != 0) {
#if DEBUG
              NSCAssert1(result == 0, @"GDataXMLNode XPath namespace %@ issue",
                        prefix);
      ...
      if (xpathObj) {
        ...
        if (nodeSet) {
          ...
          for (NSInteger index = 0; index < nodeSet->nodeNr; index++) {
...
- (NSString *)description {
  NSInteger nodeType = (xmlNode_ ? (NSInteger)xmlNode_->type : -1);

  return [NSString stringWithFormat:@"%@ %p: {type:%zi name:%@ xml:\"%@\"}",
...
- (id)initWithXMLString:(NSString *)str error:(NSError **)error {
  ...
  if (self) {
    ...
    // NOTE: We are assuming a string length that fits into an NSInteger
    xmlDocPtr doc = xmlReadMemory(utf8Str, (NSInteger)strlen(utf8Str), NULL, // URL
                                  NULL, // encoding
  ...
  if (nodeToFix->ns == NULL) {
    ...
    if (localName != NULL) {
      if (prefix != NULL) {
        ...
        NSInteger prefixLen = xmlStrlen(prefix);
        if (prefixLen > 2
            && prefix[0] == '{'
            && prefix[prefixLen - 1] == '}') {
...
- (id)initWithData:(NSData *)data options:(unsigned int)mask error:(NSError **)error {
  ...
  if (self) {
    ...
    // NOTE: We are assuming [data length] fits into an NSInteger.
    xmlDoc_ = xmlReadMemory([data bytes], (NSInteger)[data length], baseURL, encoding,
...
static Boolean StringCacheKeyEqualCallBack(const void *str1, const void *str2) {
  ...
  NSInteger result = xmlStrcmp(str1, str2);
  return (result == 0);
}
