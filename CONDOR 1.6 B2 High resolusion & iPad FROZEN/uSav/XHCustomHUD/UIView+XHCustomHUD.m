...
//
//  Modified by Luca 2015-01-30 Added some return value
...
- (XHHUDView *)showHUDWithText:(NSString *)text hudType:(XHHUDType)hudType animationType:(XHHUDAnimationType)animationType {
    ...
    XHHUDView *alert = [self showHUDWithText:text hudType:hudType animationType:animationType delay:0];
    return alert;
}
...
- (XHHUDView *)showHUDWithText:(NSString *)text hudType:(XHHUDType)hudType animationType:(XHHUDAnimationType)animationType delay:(NSTimeInterval)delay {
    XHHUDView *alert = [self showHUDWithText:text hudSize:CGSizeMake(90, 90) hudType:hudType animationType:animationType delay:delay];
    return alert;
}
...
- (XHHUDView *)showHUDWithText:(NSString *)text
    ...
    hudView.center = CGPointMake([UIScreen mainScreen].bounds.size.width/2, [UIScreen mainScreen].bounds.size.height/2 - 40);
    hudView.text = text;
...
- (void)dismissHUD:(XHHUDView *)hudView delay:(NSTimeInterval)delay {
    ...
    //unlock the screen
    [[hudView superview] setUserInteractionEnabled:YES];
...
- (void)dismissWithClickedButtonIndex:(NSInteger)index animated:(BOOL)animated {
    ...
    [self dismissHUD:(XHHUDView *)self delay:0];
}
...
- (BOOL)isVisible {
    ...
    for (NSInteger i = 0; i < [self.superview.subviews count]; i ++) {
        ...
        if ([self.superview.subviews[i] isKindOfClass:[XHHUDView class]]) {
            return YES;
        }
    }
    return NO;
