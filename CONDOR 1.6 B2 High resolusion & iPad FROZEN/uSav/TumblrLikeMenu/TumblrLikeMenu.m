...
#define kFloatMenuItemDisappearDuration  (0.5f)
#define kFloatTipLabelAppearDuration     (0.25f)
#define kFloatTipLabelHeight             (26.0f)
...
- (id)initWithFrame:(CGRect)frame subMenus:(NSArray *)menus tip:(NSString *)tip
{
    ...
    if (self)
    {
        ...
        if (tip)
        {
            self.tipLabel = [[UILabel alloc] initWithFrame:CGRectMake(CGRectGetWidth(frame)/2 - CGRectGetWidth(frame)/2.99/2, CGRectGetHeight(frame) - ((30.0/568.0)*self.bounds.size.height), CGRectGetWidth(frame)/2.99, (kFloatTipLabelHeight/568.0)*self.bounds.size.height)];
        ...
        //最后添加标题
        self.topTitle = [[UILabel alloc] initWithFrame:CGRectMake(self.bounds.size.width/1.9 - (210/2), self.bounds.size.height/7.5, 196, (30.0/568.0)*self.bounds.size.height)];
        
        //如果是iPhone4，则稍微放大
        if([UIScreen mainScreen].bounds.size.height == 480){
            self.topTitle = [[UILabel alloc] initWithFrame:CGRectMake(self.bounds.size.width/1.9 - (210/2), self.bounds.size.height/7.5, 196, (34.0/568.0)*self.bounds.size.height)];
        }
        
        //如果是iPad，则微调位置
        if([[[UIDevice currentDevice].model substringToIndex:4] isEqualToString:@"iPad"]){
            [self.topTitle setFrame:CGRectMake(self.bounds.size.width/1.9 - (210/2), self.bounds.size.height/8, 196, 36)];
            [self.tipLabel setFrame:CGRectMake(CGRectGetWidth(frame)/2 - CGRectGetWidth(frame)/4.2/2, CGRectGetHeight(frame) - ((30.0/568.0)*self.bounds.size.height), CGRectGetWidth(frame)/4.2, (kFloatTipLabelHeight/568.0)*self.bounds.size.height)];
        }
...
- (void)setupSubmenus
{
    ...
    for (NSInteger i = 0; i < 3; ++i)
    {
        ...
        for (NSInteger j = 0; j < 3; ++j)
        {
            ...
            subMenu.center = CGPointMake(CGRectGetWidth(self.frame)*(103.0/320.0) * j + CGRectGetWidth(self.frame)*(62.0/320.0), CGRectGetHeight(self.frame) + i * 125 + 60);
            //第三行下移一行，并且微调位置
            if (i == 2) {
                subMenu.center = CGPointMake(CGRectGetWidth(self.frame)*(103.0/320.0) * j + CGRectGetWidth(self.frame)*(62.0/320.0), CGRectGetHeight(self.frame) + (i + 1) * ((125/568.0)*screenHeight) - ((30/568.0)*screenHeight));
            }
            //如果是iPhone4，第三行则轻微修正位置
            if (screenHeight == 480 && i == 2) {
                subMenu.center = CGPointMake(CGRectGetWidth(self.frame)*(103.0/320.0) * j + CGRectGetWidth(self.frame)*(62.0/320.0), CGRectGetHeight(self.frame) + (i + 1) * ((125/568.0)*screenHeight) - ((0/568.0)*screenHeight));
            }
            ...
            if([[[UIDevice currentDevice].model substringToIndex:4] isEqualToString:@"iPad"]){
                ...
                //第三行下移一行，并且微调位置
                if (i == 2) {
                    subMenu.center = CGPointMake(CGRectGetWidth(self.frame)*(101.0/320.0) * j + CGRectGetWidth(self.frame)*(62.0/320.0), CGRectGetHeight(self.frame) + (i + 1) * ((125/568.0)*screenHeight) - ((56/568.0)*screenHeight));
                }
            }   //i*x为紧密度，+的数字为整体位移
...
//    subMenu.center = CGPointMake(106 * 2 + 62, CGRectGetHeight(self.frame) + 1 * 125 + 20);
//    //如果是iPad，则微调位置
//    if([[[UIDevice currentDevice].model substringToIndex:4] isEqualToString:@"iPad"]){
//        subMenu.center = CGPointMake(106 * 2 + 62, CGRectGetHeight(self.frame) + 1 * 100 + 44);
//    }
//    if (NULL == subMenu.selectBlock)
//    {
//        __weak TumblrLikeMenu *weakSelf = self;
...
- (void)appear
{
    ...
    if (self.tipLabel)
    {
        ...
        if ([[BundleLocalization sharedInstance].language isEqualToString:@"zh-Hans"]) {
            UIImageView *tipBackground = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"Function_logout_SC_A"]];
            tipBackground.frame = CGRectMake(24, -4, self.tipLabel.frame.size.width - 40, self.tipLabel.frame.size.height);
            [self.tipLabel addSubview:tipBackground];
        } else {
            UIImageView *tipBackground = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"Function_logout_EN_A"]];
            tipBackground.frame = CGRectMake(4, -4, self.tipLabel.frame.size.width, self.tipLabel.frame.size.height);
            [self.tipLabel addSubview:tipBackground];
        }
...
- (void)disappear
{
    ...
    //整体View
    [UIView animateWithDuration:kFloatMenuItemDisappearDuration delay:0.25 options:UIViewAnimationOptionCurveEaseInOut animations:^{
