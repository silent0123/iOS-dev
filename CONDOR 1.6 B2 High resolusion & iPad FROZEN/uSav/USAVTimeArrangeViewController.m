...
@interface USAVTimeArrangeViewController () {
    NSInteger numberOfRowForDisplayPicker;
}
...
- (void)viewDidLoad {
    ...
    */
    ...
    numberOfRowForDisplayPicker = 2;    //initially is 2, when picker displayed, it's 3
    tableButtonIndex = -1;
    ...
    //if is from secure chatting, cannot change this button
    if ([self.COPeoplePickerTimeDelegate isKindOfClass:[USAVSecureChatViewController class]]) {
        [self.allowCopySwitch setOn:NO];
        self.allowSaveDecryptCopy = 0;
        [self.allowCopySwitch setEnabled:NO];
    }
    ...
    self.numPicker = [[UIPickerView alloc] initWithFrame:CGRectMake(0, 0 , [[UIScreen mainScreen] bounds].size.width, 100)];
    ...
    //新的date picker
    self.datePicker_2 = [[UIDatePicker alloc] initWithFrame:CGRectMake(0, 0 , [[UIScreen mainScreen] bounds].size.width, 100)];
    [self.datePicker_2 addTarget:self action:@selector(datePickerChanged:) forControlEvents:UIControlEventValueChanged];
    [self.datePicker_2 addTarget:self action:@selector(datePickerEditBegin:) forControlEvents:UIControlEventEditingDidBegin];
    [self.datePicker_2 addTarget:self action:@selector(datePickerEditEnd:) forControlEvents:UIControlEventEditingDidEnd];

    
    
    self.numPicker.backgroundColor = [UIColor clearColor];
    self.datePicker.backgroundColor = [UIColor clearColor];
    self.datePicker_2.backgroundColor = [UIColor clearColor];
    
    //datePicker的最小时间为当前
    self.datePicker.minimumDate = [NSDate date];
    self.datePicker_2.minimumDate = [NSDate date];
    //修改datepicker位置
    [self.datePicker setFrame:[self.numPicker frame]];
...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView {
    ...
    if(self.allowSaveDecryptCopy) {
        ...
        if (self.allowSaveDecryptCopy) {
            [self.datePicker_2 setHidden:YES];
            [self.numPicker setHidden:YES];
        }
        return 1;
    } else {
        ...
        //no selection, hide
        if (tableButtonIndex == -1) {
            [self.datePicker_2 removeFromSuperview];
            [self.numPicker removeFromSuperview];
        } else if (tableButtonIndex == 0 || tableButtonIndex == 1) {
            [self.datePicker_2 removeFromSuperview];
        } else {
            [self.numPicker removeFromSuperview];
        }
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    } else if ((tableButtonIndex == 0 || tableButtonIndex == 1) && section == 1) {
        
        
        return numberOfRowForDisplayPicker;
        
    } else if ((tableButtonIndex == 2 || tableButtonIndex == 3) && section == 2){
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (numberOfRowForDisplayPicker == 3) {
        switch (tableButtonIndex) {
            case 0:
                if (section == 1 && row == 1) {
                    return 160;
                }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    switch (section) {
        ...
        case 1: {
            ...
            } else if (numberOfRowForDisplayPicker == 3 && tableButtonIndex == 0) {
                
                
                //if selected viewing limit
                if (row == 1) {
                    cell.textLabel.text = @"";
                    cell.detailTextLabel.text = @"";
                    cell.accessoryType = UITableViewCellAccessoryNone;
                    ...
                    [cell.contentView addSubview:self.numPicker];
            ...
            } else if (numberOfRowForDisplayPicker == 3 && tableButtonIndex == 1) {
                
                
                if (row == 2) {
                    //if selected viewing duration
                    cell.textLabel.text = @"";
                    cell.detailTextLabel.text = @"";
                    cell.accessoryType = UITableViewCellAccessoryNone;
                    [self.numPicker setHidden:NO];
            ...
            } else if (numberOfRowForDisplayPicker == 2 && row == 1) {
        ...
        case 2: {
            ...
            } else if (numberOfRowForDisplayPicker == 3 && tableButtonIndex == 2) {
                //selected start time
                if (row == 1) {
                    cell.textLabel.text = @"";
                    cell.detailTextLabel.text = @"";
                    cell.accessoryType = UITableViewCellAccessoryNone;
                    [self.datePicker_2 setHidden:NO];
                    [self.datePicker_2 setFrame:CGRectMake(0, 0, cell.frame.size.width, cell.frame.size.height)];
                    for (NSInteger i = 0; i < [cell.contentView.subviews count]; i ++) {

                        [[cell.contentView.subviews objectAtIndex:i] removeFromSuperview];
                    }
                    
                    [cell.contentView addSubview:self.datePicker_2];
            ...
            } else if (numberOfRowForDisplayPicker == 3 && tableButtonIndex == 3) {
                //selected end time
                if (row == 2) {
                    cell.textLabel.text = @"";
                    cell.detailTextLabel.text = @"";
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    switch (section) {
        ...
        case 1:
            ...
            } else if (tableButtonIndex == 3) {
                [self tableView:tableView didSelectRowAtIndexPath:[NSIndexPath indexPathForRow:1 inSection:2]];
            }
            ...
            if (row == 0) {
                ...
                if (tableButtonIndex == 0) {
                    ...
                    numberOfRowForDisplayPicker = 2;
                    tableButtonIndex = -1;
 
                } else {
                    
                    tableButtonIndex = 0;
                    numberOfRowForDisplayPicker = 3;
            ...
            if(tableButtonIndex == 0 || tableButtonIndex == 1 || tableButtonIndex == -1) {
        ...
        case 2:
            ...
            [self.datePicker_2 setDate:[NSDate date]];//默认值
            ...
            } else if (tableButtonIndex == 1) {
                [self tableView:tableView didSelectRowAtIndexPath:[NSIndexPath indexPathForRow:1 inSection:1]];
            }
            ...
            [self.datePicker_2 reloadInputViews];

            
            if(tableButtonIndex == 2 || tableButtonIndex == 3 || tableButtonIndex == -1) {
                [self.tableView reloadSections:[NSIndexSet indexSetWithIndex:2] withRowAnimation:UITableViewRowAnimationAutomatic];
            } else {
                [self.tableView reloadSections:[NSIndexSet indexSetWithIndex:1] withRowAnimation:UITableViewRowAnimationAutomatic];
                [self.tableView reloadSections:[NSIndexSet indexSetWithIndex:2] withRowAnimation:UITableViewRowAnimationAutomatic];
...
- (IBAction)datePickerChanged:(id)sender {
    ...
    NSString *displayFinalDate = [displayDateFormatter stringFromDate:self.datePicker_2.date];    //用户可读的日期，没有秒
    
    //选择的是startTime
    if (tableButtonIndex == 2) {
        self.finalStartTime = displayFinalDate;
        self.finalStartDate = self.datePicker_2.date; //这个用来保存NSDate，在confirm的时候确定是否起始早于结束
        
        //没有设置结尾而设置了开头, 把结尾设置为开头之后的1分钟
        if (self.finalEndDate == nil || [self.finalEndTime isEqualToString:@"No Limit"]) {
            self.finalEndTime = [displayDateFormatter stringFromDate:[self.datePicker_2.date dateByAddingTimeInterval:60]];
            self.finalEndDate = [self.datePicker_2.date dateByAddingTimeInterval:60];
        }
        ...
        self.finalEndDate = self.datePicker_2.date;
...
- (NSAttributedString *)pickerView:(UIPickerView *)pickerView attributedTitleForRow:(NSInteger)row forComponent:(NSInteger)component {
    ...
    if (tableButtonIndex == 0) {
        if (row == 0) {
            stringToDisplay = NSLocalizedString(@"Times", nil);
        } else if (row == 1){
            
            stringToDisplay = NSLocalizedString(@"No Limit", nil);
            
        } else {
            stringToDisplay = [NSString stringWithFormat:@"%zi", row - 1];
        }
    } else {
        //选阅读时间，显示Title, 0-24 | title, 0-60 | title, 0-60
        switch (component) {
            case 0:
                if (row == 0) {
                    ...
                    stringToDisplay = NSLocalizedString(@"Hour", nil);
            ...
            case 1:
                if (row == 0) {
                    stringToDisplay = NSLocalizedString(@"Minute", nil);
            ...
            default:
                if (row == 0) {
                    ...
                    stringToDisplay = NSLocalizedString(@"Second", nil);
    ...
    NSMutableAttributedString *str = [[NSMutableAttributedString alloc] initWithString:stringToDisplay];
    [str addAttribute:NSFontAttributeName value:[UIFont fontWithName:@"HelveticaNeue" size:13.0] range:NSMakeRange(0, [stringToDisplay length])];
