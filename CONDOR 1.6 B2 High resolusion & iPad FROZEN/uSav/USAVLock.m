...
- (BOOL)isLoginSessionTimeOut {
    ...
    double sessionTimeout = [[[NSUserDefaults standardUserDefaults] objectForKey:@"sessionTimeOutInterval"] floatValue];
    
    double diff = [num doubleValue] - [[[NSUserDefaults standardUserDefaults] objectForKey:@"baseTime"] doubleValue];
    
    NSInteger timeoutEnabled = [[NSUserDefaults standardUserDefaults] integerForKey:@"isSessionTimeOut"];
    
    NSLog(@"Session Timeout Enabled? %zi, hasTimeout? %zi", timeoutEnabled,(diff > sessionTimeout || diff < 0));
    if (timeoutEnabled && (diff > sessionTimeout || diff < 0)) {
        return true;
    } else {
        return false;
    }
...
- (void)setLoginSessionTimeOutOff {
    //sessionTimeOut是LoginStatus相关的
    [[NSUserDefaults standardUserDefaults] setInteger:0 forKey:@"isSessionTimeOut"];
}
...
- (NSString*)getLockTimeStr {
    switch ([self getLockTime]) {
        case 0:return NSLocalizedString(@"Always", @"");
...
- (void)setLockTime:(NSInteger )lockTime {
    switch (lockTime) {
        case 0:[self setTimeOut:[NSNumber numberWithInt:0]]; break;
