...
- (void)viewDidLoad
{
    ...
    //process image to full size of screen
    UIGraphicsBeginImageContext(self.scrollView.frame.size);
    [[UIImage imageNamed:@"Function_bg"] drawInRect:self.scrollView.bounds];
    UIImage *image = UIGraphicsGetImageFromCurrentImageContext();
    UIGraphicsEndImageContext();
    self.scrollView.backgroundColor = [UIColor colorWithPatternImage:image];

    
    self.loginRegBtn.backgroundColor = [UIColor colorWithWhite:0.02 alpha:0.8];
    self.cancelBtn.backgroundColor = [UIColor colorWithWhite:0.02 alpha:0.8];
    self.askToRegisterBtn.backgroundColor = [UIColor colorWithWhite:0.02 alpha:0.8];
    self.btnForgetPassword.backgroundColor = [UIColor colorWithWhite:0.02 alpha:0.8];
    ...
    //if there is an account logined before
    //put it into the email textfield
    if ([[NSUserDefaults standardUserDefaults] objectForKey:@"previousAccount"] != nil) {
        self.userEmailTextField.text = [[NSUserDefaults standardUserDefaults] objectForKey:@"previousAccount"];
    }
...
//    [self.btnForgetPassword setBackgroundColor:[UIColor colorWithRed:(30/255) green:(144/255) blue:(255/255) alpha:0.5]];
    ...
    //create two new field to input firstname & lastname
    self.firstNameTextField = [[UITextField alloc] initWithFrame:CGRectMake(self.reenterPwTextField.frame.origin.x, self.reenterPwTextField.frame.origin.y + 46, self.reenterPwTextField.frame.size.width - 80, self.reenterPwTextField.frame.size.height)];
    self.firstNameTextField.borderStyle = UITextBorderStyleRoundedRect;
    self.firstNameTextField.font = [UIFont systemFontOfSize:14];
    self.firstNameTextField.textColor = self.userEmailTextField.textColor;
    self.firstNameTextField.backgroundColor = self.userEmailTextField.backgroundColor;
    self.firstNameTextField.returnKeyType = UIReturnKeyDone;
    self.firstNameTextField.placeholder = NSLocalizedString(@"Firstname", nil);
    self.firstNameTextField.delegate = self;
    
    self.lastNameTextField = [[UITextField alloc] initWithFrame:CGRectMake(self.reenterPwTextField.frame.origin.x + (self.reenterPwTextField.frame.size.width - 80) + 10, self.reenterPwTextField.frame.origin.y + 46, self.reenterPwTextField.frame.size.width - 80, self.reenterPwTextField.frame.size.height)];
    self.lastNameTextField.borderStyle = UITextBorderStyleRoundedRect;
    self.lastNameTextField.font = [UIFont systemFontOfSize:14];
    self.lastNameTextField.textColor = self.userEmailTextField.textColor;
    self.lastNameTextField.backgroundColor = self.userEmailTextField.backgroundColor;
    self.lastNameTextField.returnKeyType = UIReturnKeyDone;
    self.lastNameTextField.placeholder = NSLocalizedString(@"Lastname", nil);
...
- (void)keyboardWillShow:(NSNotification *)notification {
    ...
    if (!self.hasMovedUp) {
        ...
        if (self.isLoginState) {
            [self animateView:self.view up:YES forHeight:movementHeight - 30];
            self.hasMovedUp = YES;
            previousHeight = movementHeight - 30;
        } else {
            [self animateView:self.view up:YES forHeight:movementHeight];
            self.hasMovedUp = YES;
            previousHeight = movementHeight;
        }
...
- (void)keyboardWillHide:(NSNotification *)notification {
    ...
    if (self.hasMovedUp) {
        ...
        if (self.isLoginState) {
            [self animateView:self.view up:NO forHeight:previousHeight];
            self.hasMovedUp = NO;
        } else {
            [self animateView:self.view up:NO forHeight:previousHeight];
            self.hasMovedUp = NO;
...
- (void)animateView: (UIView *)view up:(BOOL)up forHeight: (CGFloat)distance {
    ...
    self.headerLogo.alpha = up ? 0.2 : 1;
    ...
    if (self.isLoginState == TRUE) {
        ...
        [UIView animateWithDuration:.5
                         ...
                         animations:^{
                             ...
                             frame = self.firstNameTextField.frame;
                             frame.size.height = 31.0f;
                             frame.origin.y += 45;
                             self.firstNameTextField.frame = frame;
                             self.firstNameTextField.text = @""; //清空firstname
                             
                             frame = self.lastNameTextField.frame;
                             frame.size.height = 31.0f;
                             frame.origin.y += 45;
                             self.lastNameTextField.frame = frame;
                             self.lastNameTextField.text = @""; //清空firstname
...
- (IBAction)loginRegBtnPressed:(id)sender {
    ...
    for (NSInteger i = 0; i < [[self.scrollView subviews] count]; i ++) {
        ...
        if ([[[self.scrollView subviews] objectAtIndex:i] isFirstResponder]) {
            [[[self.scrollView subviews] objectAtIndex:i] resignFirstResponder];
        }
    ...
    if (self.isLoginState) { // button being used for login
        ...
        paramElement = [GDataXMLNode elementWithName:@"firstName" stringValue:[NSString stringWithFormat:@"%@",self.firstNameTextField.text]];
        [requestElement addChild:paramElement];
        paramElement = [GDataXMLNode elementWithName:@"lastName" stringValue:[NSString stringWithFormat:@"%@", self.lastNameTextField.text]];
        [requestElement addChild:paramElement];
