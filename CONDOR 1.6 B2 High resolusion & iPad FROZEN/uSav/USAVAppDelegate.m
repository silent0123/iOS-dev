...
//    NSLog(@"iCloud Drive established:%@", url);
    
    NSArray *paths = NSSearchPathForDirectoriesInDomains(
                                                         NSDocumentDirectory, NSUserDomainMask, YES);
    NSFileManager *fileManager = [NSFileManager defaultManager];
    
    //if is HOME PAGE version
    if ([self.window.rootViewController isKindOfClass:[UINavigationController class]]) {
        _navigationController = (UINavigationController *)self.window.rootViewController ;

    } else {
    //if is chatting central version
        _navigationController = (UINavigationController *)[[(UITabBarController *)self.window.rootViewController viewControllers] objectAtIndex:0];
...
- (void)reloadLocalization {
    ...
    if ([[[NSUserDefaults standardUserDefaults] objectForKey:@"userLanguage"] length] == 0) {
        
        //如果没有设置，则为系统默认
        [[BundleLocalization sharedInstance] setLanguage: [[[NSUserDefaults standardUserDefaults] objectForKey:@"AppleLanguages"] objectAtIndex:0]];
        
    } else {
        
        [[BundleLocalization sharedInstance] setLanguage: [[NSUserDefaults standardUserDefaults] objectForKey:@"userLanguage"]];
...
//    }
    ...
    if (JAILBREAK_DETECT) {
        if ([self isJailBreak]) {
            ...
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"CONDOR Security Detection", nil) message:NSLocalizedString(@"Your device has been jailbroken. To protect your data, CONDOR Phone Armour has stopped running.  Please correct the problem ASAP.", nil) delegate:self cancelButtonTitle:NSLocalizedString(@"Dismiss", nil) otherButtonTitles:nil, nil];
