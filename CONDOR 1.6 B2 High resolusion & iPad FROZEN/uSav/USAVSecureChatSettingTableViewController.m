...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [self.navigationItem setTitle:NSLocalizedString(@"Restrictions", nil)];
}
...
- (void)viewDidLoad {
    ...
    if ([[NSUserDefaults standardUserDefaults] integerForKey:@"DefaultDuration"] >= 0) {
        
        selectedDuration = [[NSUserDefaults standardUserDefaults] integerForKey:@"DefaultDuration"];
    }
...
- (NSString *)tableView:(UITableView *)tableView titleForHeaderInSection:(NSInteger)section {
    ...
    if (section == 0) {
        ...
        return NSLocalizedString(@"Default Viewing Duration", nil);
    } else {
        return nil;
    }
}
...
- (NSString *)tableView:(UITableView *)tableView titleForFooterInSection:(NSInteger)section {
    if (section == 1) {
        return NSLocalizedString(@"Default viewing limit and duration will be assigned to receiver automatically during message sending", nil);
    } else {
        return nil;
    }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        ...
        if (indexPath.row == 0) {
            ...
            cell.textLabel.text = NSLocalizedString(@"No Limit", nil);
            if (selectedLimit == 0) {
                cell.accessoryType = UITableViewCellAccessoryCheckmark;
            } else {
                cell.accessoryType = UITableViewCellAccessoryNone;
        ...
        if (indexPath.row == 0) {
            cell.textLabel.text = [NSString stringWithFormat:NSLocalizedString(@"%zi Second", nil), 1];
            ...
            cell.textLabel.text = [NSString stringWithFormat:NSLocalizedString(@"%zi Seconds", nil), 5];
            ...
            cell.textLabel.text = [NSString stringWithFormat:NSLocalizedString(@"%zi Seconds", nil), 10];
            ...
            cell.textLabel.text = [NSString stringWithFormat:NSLocalizedString(@"%zi Seconds", nil), 30];
...
- (IBAction)doneBtnPressed:(id)sender {
    ...
    if (selectedDuration == 0 && selectedLimit == 0) {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"Warning", nil) message:[NSString stringWithFormat:NSLocalizedString(@"Default setting of viewing limit and duration per viewing will be changed to unlimited times and unlimited seconds", nil), selectedLimit] delegate:self cancelButtonTitle:NSLocalizedString(@"CancelKey", nil) otherButtonTitles:NSLocalizedString(@"OkKey", nil), nil];
        alert.delegate = self;
        alert.tag = CONFIRM_LIMIT_CHANGE_ALERT_TAG;
        [alert show];
    }else if (selectedDuration == 0 && selectedLimit != 0) {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"Warning", nil) message:[NSString stringWithFormat:NSLocalizedString(@"Default setting of viewing limit and duration per viewing will be changed to %zi time(s) and unlimited seconds", nil), selectedLimit] delegate:self cancelButtonTitle:NSLocalizedString(@"CancelKey", nil) otherButtonTitles:NSLocalizedString(@"OkKey", nil), nil];
        alert.delegate = self;
        alert.tag = CONFIRM_LIMIT_CHANGE_ALERT_TAG;
        [alert show];

    } else if (selectedDuration != 0 && selectedLimit == 0) {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"Warning", nil) message:[NSString stringWithFormat:NSLocalizedString(@"Default setting of viewing limit and duration per viewing will be changed to unlimited times and %zi second(s)", nil), selectedDuration] delegate:self cancelButtonTitle:NSLocalizedString(@"CancelKey", nil) otherButtonTitles:NSLocalizedString(@"OkKey", nil), nil];
        alert.delegate = self;
        alert.tag = CONFIRM_LIMIT_CHANGE_ALERT_TAG;
        [alert show];

    } else {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"Warning", nil) message:[NSString stringWithFormat:NSLocalizedString(@"Default setting of viewing limit and duration per viewing will be changed to %zi time(s) and %zi second(s)", nil), selectedLimit, selectedDuration] delegate:self cancelButtonTitle:NSLocalizedString(@"CancelKey", nil) otherButtonTitles:NSLocalizedString(@"OkKey", nil), nil];
        alert.delegate = self;
        alert.tag = CONFIRM_LIMIT_CHANGE_ALERT_TAG;
        [alert show];
...
- (void)alertView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex {
    ...
    if (alertView.tag == CONFIRM_LIMIT_CHANGE_ALERT_TAG) {
        if (buttonIndex == 1) {
            ...
            [[NSUserDefaults standardUserDefaults] setInteger:selectedDuration forKey:@"DefaultDuration"];
