...
@interface USAVSecureChatViewController () {
    ...
    NSInteger TF_newLimitFromRestrictions;
    NSInteger TF_newDurationFromRestrictions;
    
    NSString * TF_newStartTimeFromRestrictions;
    NSString * TF_newEndTimeFromRestrictions;
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [self updateRestrictionBtn];
    self.tableView.frame = CGRectMake(0, 0, [[UIScreen mainScreen] bounds].size.width, self.tableView.frame.size.height);
...
- (void)viewDidLoad {
    ...
    endMessageIndex = 0;
    playingRow = -1;
    TF_newDurationFromRestrictions = -1;
    TF_newLimitFromRestrictions = -1;
    TF_newStartTimeFromRestrictions = TF_newEndTimeFromRestrictions = nil;
    ...
    // {"duration": xx} -- viewing duration of this message
    // {...} reserved
...
#pragma mark 清空decrypt - 启用
    ...
    self.timerList = [[NSMutableArray alloc] initWithCapacity:0];
    ...
    if([[[UIDevice currentDevice].model substringToIndex:4] isEqualToString:@"iPad"]){
        [self.voiceRecordingCustomView setFrame:CGRectMake(self.voiceRecordingCustomView.frame.origin.x, self.voiceRecordingCustomView.frame.origin.y - 40, self.voiceRecordingCustomView.frame.size.width, self.voiceRecordingCustomView.frame.size.height)];
    }
    originalPointOfRecordingCustomView = CGPointMake([UIScreen mainScreen].bounds.size.width/2, [UIScreen mainScreen].bounds.size.height/2 - 40);
    ...
    self.restrictionsBtn.frame = CGRectMake(0, [UIScreen mainScreen].bounds.size.height - 99 - 28, [UIScreen mainScreen].bounds.size.width, 28);
    [self.view bringSubviewToFront:self.inputMessageView];
    [self.view bringSubviewToFront:self.restrictionsBtn];
...
- (void)updateRestrictionBtn {
    
    //retriction button
    NSInteger newLimit;
    NSInteger newDuration;
    
    if (TF_newLimitFromRestrictions != -1 || TF_newDurationFromRestrictions != -1) {
        newLimit = TF_newLimitFromRestrictions;
        newDuration = TF_newDurationFromRestrictions;
    } else {
        newLimit = [[NSUserDefaults standardUserDefaults] integerForKey:@"DefaultLimit"];
        newDuration = [[NSUserDefaults standardUserDefaults] integerForKey:@"DefaultDuration"];
    }
    
    
    NSString *titleOfRestriction = [NSString stringWithFormat:NSLocalizedString(@"Viewing Restrictions: %zi times | %zi seconds", nil), newLimit, newDuration];

    
    if (newLimit == 0 && newDuration != 0) {
        
        titleOfRestriction = [NSString stringWithFormat:NSLocalizedString(@"Viewing Restrictions: unlimit times | %zi seconds", nil), newDuration];
        
    } else if (newLimit != 0 && newDuration == 0) {
        
        titleOfRestriction = [NSString stringWithFormat:NSLocalizedString(@"Viewing Restrictions: %zi times | unlimit seconds", nil), newLimit];
        
    } else if (newLimit == 0 && newDuration == 0){
        
        titleOfRestriction = NSLocalizedString(@"Viewing Restrictions: No Limit", nil);
    }
    
    TF_newLimitFromRestrictions = newLimit;
    TF_newDurationFromRestrictions = newDuration;   //prepare for pass value
    
    [self.restrictionsBtn setTitle:titleOfRestriction forState:UIControlStateNormal];
}
...
- (void)tableViewScrollToButtom:(UITableView *)tableView {
    ...
    if([self.resultArray count] > 5) {
        ...
        [tableView setFrame:CGRectMake(0, 0, tableView.frame.size.width, self.restrictionsBtn.frame.origin.y)];
...
- (void)keyboardWillShow:(NSNotification *)notification {
    ...
    if (self.keyboardRect.origin.y != self.view.bounds.size.height && !inputIsShowed && !isInputFileBtnPressed) {
        ...
        [self animateView:self.restrictionsBtn up:YES forHeight:self.keyboardRect.size.height isPosition:YES];
        ...
        [self animateView:self.restrictionsBtn up:NO forHeight:previousHeight isPosition:YES];
...
- (void)keyboardWillHide: (NSNotification *)notification {
    ...
    [self animateView:self.restrictionsBtn up:NO forHeight:self.keyboardRect.size.height isPosition:YES];
...
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text {
    ...
    if ([text isEqualToString:@"\n"]) {
        ...
        if (isCreateSuccessful) {
            ...
            NSMutableDictionary *dic = [NSMutableDictionary dictionaryWithObjectsAndKeys:[[USAVClient current] emailAddress], @"sender", sendString, @"content", [dateFormatter stringFromDate:[[self.fileManager attributesOfItemAtPath:self.filePath error:nil] objectForKey:NSFileCreationDate]], @"time", @"1", @"type", [[NSArray alloc] init], @"permissionList", self.filePath, @"decryptedFilePath", self.filePath, @"encryptedFilePath", nil];
            [dic setObject:[NSNumber numberWithInteger:0] forKey:@"duration"];
...
- (UIView *)bubbleView: (USAVSecureChatBubbleTableViewCell *)singleCell and: (NSString *)text from:(BOOL)fromSelf withIndexRow:(NSInteger)indexRow withPosition:(NSInteger)position andFont:(UIFont *)font andTextColor: (UIColor *)textColor type: (NSInteger)typeOfMessage duration: (NSInteger)duration{
    ...
    if(fromSelf) {
        singleCell.bubbleView.frame = CGRectMake([[UIScreen mainScreen] bounds].size.width - position - (singleCell.textBubbleLabel.frame.size.width + 30.0f), 38.0f, singleCell.textBubbleLabel.frame.size.width+30.0f, singleCell.textBubbleLabel.frame.size.height+ 26.0f);
    }
    else {
    ...
    //duration progress
    BOOL isProgressAdded = NO;
    for (NSInteger i = 0; i < [[singleCell.bubbleImage subviews] count]; i ++) {
        if ([[[singleCell.bubbleImage subviews] objectAtIndex:i] isKindOfClass:[UIProgressView class]]) {
            
            // == to avoid the duplicating of progress view
            // otherwise, every time when reloading data, new progress view will be added into the cell
            [[[singleCell.bubbleImage subviews] objectAtIndex:i] removeFromSuperview];
            isProgressAdded = NO;
        }
    }

    if (duration > 0 && !isProgressAdded) {
        UIProgressView *durationProgress = [[UIProgressView alloc] initWithProgressViewStyle:UIProgressViewStyleDefault];
        durationProgress.frame = CGRectMake(singleCell.textBubbleLabel.frame.origin.x, singleCell.bubbleImage.frame.size.height - 5, singleCell.textBubbleLabel.frame.size.width - 6, 4);
        durationProgress.backgroundColor = [UIColor clearColor];
        durationProgress.trackTintColor = [UIColor clearColor];
        durationProgress.progressTintColor = [UIColor colorWithRed:(30.0/255.0) green:(144.0/255.0) blue:(255.0/255.0) alpha:1];
        durationProgress.progress = 1.0;
        durationProgress.alpha = 0.8;
        
        [singleCell.bubbleImage addSubview:durationProgress];
        
        //timer
        //only add this timer when didn't exits
        BOOL isExists = NO;
        NSTimer *durationTimerExists = nil;
        for (NSInteger i = 0; i < [self.timerList count]; i ++) {
            if ([[[self.timerList objectAtIndex:i] objectForKey:@"indexRow"] integerValue] == indexRow && ((NSTimer *)[[self.timerList objectAtIndex:i] objectForKey:@"timer"]).valid) {
                durationTimerExists = ((NSTimer *)[[self.timerList objectAtIndex:i] objectForKey:@"timer"]);
                isExists = YES;
                break;
            }
        }
        if (!isExists) {
            NSDictionary *dictionaryOfUserInfo = [[NSDictionary alloc] initWithObjectsAndKeys:singleCell, @"cell", [NSNumber numberWithInteger:duration], @"duration",nil];
            NSTimer *durationTimer = [NSTimer scheduledTimerWithTimeInterval:1.0 target:self selector:@selector(timerTickForCell:) userInfo:dictionaryOfUserInfo repeats:YES];
            [[NSRunLoop mainRunLoop] addTimer:durationTimer forMode:NSRunLoopCommonModes];
            
            NSMutableDictionary *timerDic = [[NSMutableDictionary alloc] initWithObjectsAndKeys:durationTimer, @"timer", [NSNumber numberWithInteger:indexRow], @"indexRow", nil];
            [self.timerList addObject:timerDic];
        } else {
            //if exists, add current timer
            [[NSRunLoop mainRunLoop] addTimer:durationTimerExists forMode:NSRunLoopCommonModes];
        }
...
- (UIView *)voiceView: (USAVSecureChatBubbleTableViewCell *)singleCell and:(NSInteger)logntime from:(BOOL)fromSelf withIndexRow:(NSInteger)indexRow  withPosition:(int)position duration: (NSInteger)duration{
    ...
    if(fromSelf){
        singleCell.voiceBubbleBtn.frame =CGRectMake([[UIScreen mainScreen] bounds].size.width - position - voiceLength, 42, voiceLength, 40);
    }
    else{
...
//    imageInsert.left = fromSelf ? singleCell.voiceBubbleBtn.frame.size.width/2.5 : - MIN(singleCell.voiceBubbleBtn.frame.size.width/3.8, 18);
    ...
    //duration progress
    BOOL isProgressAdded = NO;
    for (NSInteger i = 0; i < [[singleCell.voiceBubbleBtn subviews] count]; i ++) {
        if ([[[singleCell.voiceBubbleBtn subviews] objectAtIndex:i] isKindOfClass:[UIProgressView class]]) {
            
            // == to avoid the lduplicating of progress view
            // otherwise, every time when reloading data, new progress view will be added into the cell
            [[[singleCell.voiceBubbleBtn subviews] objectAtIndex:i] removeFromSuperview];
            isProgressAdded = NO;
            break;
        }
    }
    ...
    if (duration > 0 && !isProgressAdded) {
        ...
        UIProgressView *durationProgress = [[UIProgressView alloc] initWithProgressViewStyle:UIProgressViewStyleDefault];
        durationProgress.frame = CGRectMake(20, singleCell.voiceBubbleBtn.frame.size.height - 4, singleCell.voiceBubbleBtn.frame.size.width - 34, 1);
        durationProgress.backgroundColor = [UIColor clearColor];
        durationProgress.trackTintColor = [UIColor clearColor];
        durationProgress.progressTintColor = [UIColor colorWithRed:(30.0/255.0) green:(144.0/255.0) blue:(255.0/255.0) alpha:1];
        durationProgress.progress = 1.0;
        durationProgress.alpha = 0.8;
        [singleCell.voiceBubbleBtn addSubview:durationProgress];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([[dict objectForKey:@"sender"] isEqualToString:[[USAVClient current] emailAddress]]) {
        ...
        if ([[dict objectForKey:@"type"] integerValue] == 2) {
            ...
            [cell addSubview:[self voiceView:cell and:[[dict objectForKey:@"content"] integerValue] from:YES withIndexRow:indexPath.row withPosition:55 duration:0]];
        }else{
            ...
            [cell addSubview:[self bubbleView:cell and:contentString from:YES withIndexRow:indexPath.row withPosition:55 andFont:defaultFont andTextColor:textColor type:[[dict objectForKey:@"type"] integerValue] duration:0]];
        }
        
        //Set Header
        cell.headerPhoto.frame = CGRectMake([[UIScreen mainScreen] bounds].size.width - 50, 26, 36, 36);
        ...
        cell.timeLabel.center = CGPointMake([[UIScreen mainScreen] bounds].size.width/2, cell.timeLabel.center.y);
        
        //Set Readmark
        [cell.readMark setHidden:[[dict objectForKey:@"read"] boolValue]];
        
        //Set Send to
        cell.sendToLabel.center = CGPointMake(cell.timeLabel.center.x - 10, cell.timeLabel.center.y + 20);
        cell.sendToLabel.frame = CGRectMake(cell.sendToLabel.frame.origin.x, cell.sendToLabel.frame.origin.y, [UIScreen mainScreen].bounds.size.width - (110.0/320.0)*[UIScreen mainScreen].bounds.size.width, 16);
    ...
    //friend
        ...
        //NOTE: friend cell need to add "Timer" to count the viewing duration
        NSInteger duration = [[dict objectForKey:@"duration"] integerValue];
        
        if ([[dict objectForKey:@"type"] integerValue] == 2) {
            //voice message content save length of voice
            [cell addSubview:[self voiceView:cell and:[[dict objectForKey:@"content"] integerValue] from:NO withIndexRow:indexPath.row withPosition:55 duration: duration]];
            ...
            [cell addSubview:[self bubbleView:cell and:contentString from:NO withIndexRow:indexPath.row withPosition:55 andFont:defaultFont andTextColor:textColor type:[[dict objectForKey:@"type"] integerValue] duration: duration]];
        }
...
- (void)timerTickForCell: (NSNotification *)notification {
    
    
    USAVSecureChatBubbleTableViewCell *cell = (USAVSecureChatBubbleTableViewCell *)[notification.userInfo objectForKey:@"cell"];
    NSInteger duration = [[notification.userInfo objectForKey:@"duration"] integerValue];
    
    NSInteger row = [self.tableView indexPathForCell:cell].row;
    
    
    UIProgressView *durationProgress;
    if (![cell.bubbleImage isHidden]) {
        for (NSInteger i = 0; i < [cell.bubbleImage.subviews count]; i ++) {
            if ([[cell.bubbleImage.subviews objectAtIndex:i] isKindOfClass:[UIProgressView class]]) {
                durationProgress = [cell.bubbleImage.subviews objectAtIndex:i];
                break;
            }
        }
    } else if (![cell.voiceBubbleBtn isHidden]) {
        for (NSInteger i = 0; i < [cell.voiceBubbleBtn.subviews count]; i ++) {
            if ([[cell.voiceBubbleBtn.subviews objectAtIndex:i] isKindOfClass:[UIProgressView class]]) {
                durationProgress = [cell.voiceBubbleBtn.subviews objectAtIndex:i];
                break;
            }
        }
    }
    ...
    if (durationProgress) {
        if (durationProgress.progress > 0) {
            
//            //perform in main thread, in case of UI Blocking
//            NSDictionary *dicForUpdateProgress = [[NSDictionary alloc] initWithObjectsAndKeys:durationProgress, @"progressView", [NSNumber numberWithFloat:1/duration], @"value", nil];
//            [self performSelectorOnMainThread:@selector(timerTickForProgressUpdate:) withObject:dicForUpdateProgress waitUntilDone:NO];
            
            //if is text message, ticking begin
            if (![cell.bubbleImage isHidden]) {
                //NSLog(@"=== 第%zi行，是文本，现在进度:%f", row, durationProgress.progress);
                float currentProgress = durationProgress.progress - 1.0/duration;
                [durationProgress setProgress:currentProgress animated:YES];
                
            }
            //if is voice message, wait for play
            
            if (![cell.voiceBubbleBtn isHidden] && playingRow != -1 && playingRow == row) {
                
                //NSLog(@"=== 第%zi行，是语音，现在进度:%f", row, durationProgress.progress);
                float currentProgress = durationProgress.progress - 1.0/duration;
                [durationProgress setProgress:currentProgress animated:YES];
            ...
            //stop play
            if (playingRow == row) {
                [self stopPlayVoiceMessage: cell.voiceBubbleBtn];
            }
            
            //remove progress
            [durationProgress removeFromSuperview];
            //invalidate timer
            for (NSInteger i = 0; i < [self.timerList count]; i ++) {
                
                NSDictionary *timerDic = [self.timerList objectAtIndex:i];
                
                if ([[timerDic objectForKey:@"indexRow"] integerValue] == row) {
                    NSTimer *timer = [timerDic objectForKey:@"timer"];
                    [self.timerList removeObjectAtIndex:i];
                    [timer invalidate];
                }
            }
            
            
            //initialize the bubble and resultArray
            [[self.resultArray objectAtIndex:row] setObject:[NSNumber numberWithInteger:0] forKey: @"duration"];
            [[self.resultArray objectAtIndex:row] setObject:NSLocalizedString(@"Click to display message", nil) forKey: @"content"];
            [[self.resultArray objectAtIndex:row] removeObjectForKey:@"decryptedFilePath"];
...
//- (void)timerTickForProgressUpdate: (NSDictionary *)dic{
//    UIProgressView *progressView = [dic objectForKey:@"progressView"];
...
- (void)deleteFile: (id)sender {
    ...
    if (!error) {
        ...
        [self setViewAnimationForView:self.tableView Duration:0.15f Options:UIViewAnimationOptionTransitionCrossDissolve animations:^(void){
            [self.tableView reloadData];
        } completion:nil];
    }
}
...
- (void)tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath {
    [self judgeTheCellVisibilityAndChangeContent];
}
...
- (IBAction)backBtnpressed:(id)sender {
    ...
    //invalid all timers
    for (NSInteger i = 0; i < [self.timerList count]; i ++) {
        NSTimer *timer = [[self.timerList objectAtIndex:i] objectForKey:@"timer"];
        if (timer) {
            [timer invalidate];
        }
    }
...
- (IBAction)inputVoiceBtnPressedDown:(id)sender {
    ...
    [self.voiceRecordingCustomView setAlpha:1];
...
- (IBAction)inputVoiceBtnPressedUp:(id)sender {
    ...
    //in case of animation didn't finished
    [self.voiceRecordingCustomView setHidden:NO];
    [self.voiceRecordingCustomView setHidden:YES];
...
- (void)voiceRecordingTimerTick {
 ...
 - (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
     ...
     if ([segue.identifier isEqualToString:@"SecureChatSettingSegue"]) {
         ...
         timeArrangeController.finalLimit = TF_newLimitFromRestrictions;
         timeArrangeController.finalDuration = TF_newDurationFromRestrictions;
         timeArrangeController.COPeoplePickerTimeDelegate = self;
         
        //区分iOS8
         if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 8.0) {
             if ([TF_newStartTimeFromRestrictions containsString:@"Z"]) {
                 //不是第一次进入Time页面，不加Z
                 timeArrangeController.finalStartTime = TF_newStartTimeFromRestrictions;
             } else {
                 timeArrangeController.finalStartTime = [NSString stringWithFormat:@"%@Z",TF_newStartTimeFromRestrictions];    //服务器传回来的时间少了个Z，不修改下一个页面不识别
             }
             
             if ([TF_newEndTimeFromRestrictions containsString:@"Z"]) {
                 //不是第一次进入Time页面，不加Z
                 timeArrangeController.finalEndTime = TF_newEndTimeFromRestrictions;
             } else {
                 timeArrangeController.finalEndTime = [NSString stringWithFormat:@"%@Z",TF_newEndTimeFromRestrictions];    //服务器传回来的时间少了个Z，不修改下一个页面不识别
             }
         } else {
             
             if ([TF_newStartTimeFromRestrictions rangeOfString:@"Z"].location != NSNotFound) {
                 //不是第一次进入Time页面，不加Z
                 timeArrangeController.finalStartTime = TF_newStartTimeFromRestrictions;
             } else {
                 timeArrangeController.finalStartTime = [NSString stringWithFormat:@"%@Z",TF_newStartTimeFromRestrictions];    //服务器传回来的时间少了个Z，不修改下一个页面不识别
             ...
             if ([TF_newEndTimeFromRestrictions rangeOfString:@"Z"].location != NSNotFound) {
                 //不是第一次进入Time页面，不加Z
                 timeArrangeController.finalEndTime = TF_newEndTimeFromRestrictions;
             } else {
                 timeArrangeController.finalEndTime = [NSString stringWithFormat:@"%@Z",TF_newEndTimeFromRestrictions];    //服务器传回来的时间少了个Z，不修改下一个页面不识别
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView {
    ...
    [self setViewAnimationForView:self.restrictionsBtn Duration:0.4f Options:UIViewAnimationOptionCurveEaseInOut animations:^(void){
        [self.restrictionsBtn setAlpha:0.5];
    } completion:nil];
...
- (void)scrollViewWillBeginDecelerating:(UIScrollView *)scrollView {
    [self setViewAnimationForView:self.restrictionsBtn Duration:0.4f Options:UIViewAnimationOptionCurveEaseInOut animations:^(void){
        [self.restrictionsBtn setAlpha:0.6];
    } completion:^(void){
        [self.restrictionsBtn setAlpha:0.9];
    }];
}


- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate {
    [self setViewAnimationForView:self.restrictionsBtn Duration:0.6f Options:UIViewAnimationOptionCurveEaseInOut animations:^(void){
...
- (void)judgeTheCellVisibilityAndChangeContent {
    
    //judge wheter the timer cell is visible, else hide the content when scroll away from visible view
    
    NSMutableIndexSet *indexSetToDelete = [[NSMutableIndexSet alloc] init];
    
    for (NSInteger i = 0; i < [self.timerList count]; i ++) {
        
        NSInteger indexRow = [[[self.timerList objectAtIndex:i] objectForKey:@"indexRow"] integerValue];
        
        NSIndexPath *indexPathForRow = [NSIndexPath indexPathForRow:indexRow inSection:0];
        USAVSecureChatBubbleTableViewCell *cell = (USAVSecureChatBubbleTableViewCell *)[self.tableView cellForRowAtIndexPath:indexPathForRow];
        
        
        //NSLog(@"=== %@",self.tableView.indexPathsForVisibleRows);
        if (![self.tableView.indexPathsForVisibleRows containsObject:indexPathForRow]) {
            
            
            //if is voice, stop it
            if (playingRow == indexRow) {
                //NSLog(@"=== 执行%zi, %zi",playingRow, indexRow);
                [self stopPlayVoiceMessage:cell.voiceBubbleBtn];
            ...
            //initialize the bubble and resultArray
            [[self.resultArray objectAtIndex:indexRow] setObject:[NSNumber numberWithInteger:0] forKey: @"duration"];
            [[self.resultArray objectAtIndex:indexRow] setObject:NSLocalizedString(@"Click to display message", nil) forKey: @"content"];
            [[self.resultArray objectAtIndex:indexRow] removeObjectForKey:@"decryptedFilePath"];
    ...
    //delete needed, and invalide timer
    if ([indexSetToDelete count] > 0) {
        [indexSetToDelete enumerateIndexesUsingBlock:^(NSUInteger index, BOOL *stop){
            NSTimer *timerForRow = [[self.timerList objectAtIndex:index] objectForKey:@"timer"];
            [timerForRow invalidate];
        }];
    }
    [self.timerList removeObjectsAtIndexes:indexSetToDelete];
}
...
-(void)setContactPermissionForKey:(NSString *)kid group: (NSArray *)group andFriends: (NSArray *)friend {
    ...
    for (id f in friend) {
        ...
        //新的
        GDataXMLElement * numLimit = [GDataXMLNode elementWithName:@"numLimit" stringValue:[NSString stringWithFormat:@"%zi", TF_newLimitFromRestrictions]];
        GDataXMLElement * isUser = [GDataXMLNode elementWithName:@"isUser" stringValue:@"TRUE"];
        GDataXMLElement * startTime = [GDataXMLNode elementWithName:@"startTime" stringValue:TF_newStartTimeFromRestrictions];
        GDataXMLElement * endTime = [GDataXMLNode elementWithName:@"endTime" stringValue:TF_newEndTimeFromRestrictions];
        GDataXMLElement * length = [GDataXMLNode elementWithName:@"length" stringValue: [NSString stringWithFormat:@"%zi",TF_newDurationFromRestrictions]];
...
-(void)getKeyResult:(NSDictionary*)obj {
    ...
    if ([[obj objectForKey:@"statusCode"] integerValue] == 517)
    {
        ...
        [[self.resultArray objectAtIndex:currentMessageRow] setObject:[NSNumber numberWithInteger:0] forKey:@"duration"];   //duration
            ...
            case SUCCESS:
            {
                ...
                if (rc == 0 || rc == true) {
                    ...
                    if ([[targetFullPath pathExtension] isEqualToString:@"m4a"]) {
                        ...
                        [[self.resultArray objectAtIndex:currentMessageRow] setObject:[NSNumber numberWithInteger:MIN([[obj objectForKey:@"allowedLength"] integerValue], audioDurationSeconds)] forKey:@"duration"];   //duration - min of (audio length and allowlength)
                    ...
                    //text
                        ...
                        [[self.resultArray objectAtIndex:currentMessageRow] setObject:[NSNumber numberWithInteger:[[obj objectForKey:@"allowedLength"] integerValue]] forKey:@"duration"];   //duration
...
- (void)continueNextMessageDecryption {
    ...
    fetchedMessageCount = 0;
    //if decrpt one
    if (startMessageIndex == endMessageIndex && !autoDecrypt) {
        //reload one
        [self.tableView reloadRowsAtIndexPaths:[NSArray arrayWithObjects:[NSIndexPath indexPathForRow:startMessageIndex inSection:0], nil] withRowAnimation:UITableViewRowAnimationFade];
...
- (void)establishResultDatabase {
    ...
    for (NSInteger i = 0; i < [self.keyInfoList count]; i ++) {
        ...
        NSMutableDictionary *dic = [[NSMutableDictionary alloc] initWithObjectsAndKeys:
                                    [[self.keyInfoList objectAtIndex:i] objectForKey:@"owner"], @"sender",
                                    ...
                                    NSLocalizedString(@"Click to display message", nil), @"content",
                                    nil];
        [dic setObject:[NSNumber numberWithInteger:0] forKey:@"duration"];
        [self.resultArray addObject:dic];
    }
...
- (IBAction)restrictionsBtnPressed:(id)sender {
    
    [self performSegueWithIdentifier:@"SecureChatSettingSegue" sender:self];
}

#pragma mark - delegate to receive value from restrictions
- (void)passLimit:(NSInteger)limit {
    
    TF_newLimitFromRestrictions = limit;
...
- (void)passDuration:(NSInteger)duration {
    ...
    TF_newDurationFromRestrictions = duration;
...
- (void)passTimeOfStart:(NSString *)startTime andEndTime:(NSString *)endTime {
    ...
    TF_newStartTimeFromRestrictions = startTime;
    TF_newEndTimeFromRestrictions = endTime;
}
