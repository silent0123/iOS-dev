...
- (void)viewDidLoad {
    ...
    //tableview
    self.tableView.frame = CGRectMake(0, 0, [[UIScreen mainScreen] bounds].size.width, self.tabBarController.tabBar.frame.origin.y);
    
    //hintlabel
    self.hintLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, self.view.bounds.size.width, 40)];
    self.hintLabel.center = CGPointMake(self.tableView.center.x, [UIScreen mainScreen].bounds.size.height/2 - 40);
